...
export default function RepostFail({ onClose }) {
  ...
  useEffect(() => {
    if (isStandalone) {
      ...
      const handleBeforeUnload = () => {
        channel.postMessage({ type: 'TAB_CLOSED', path: '/repost-fail' });
      };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 style={{ marginTop: 0 }}>Repost Fail</h2>
            <div style={{ display: 'flex', gap: '10px' }}>
                {!isStandalone && (
                  <button 
                  onClick={() => {
                    window.open('/repost-fail', '_blank');
                    if (onClose) onClose();
                  }}
                  style={{
                      padding: '10px 20px',
        ...
        <div style={{ marginTop: '1rem', marginBottom: '2rem', lineHeight: '1.5' }}>
            <p>
              This is a placeholder for the <strong>Repost Fail</strong> explanation.<br/>

...
export default function BranchTellerInterval({ onClose }) {
  ...
  useEffect(() => {
    if (isStandalone) {
      ...
      const handleBeforeUnload = () => {
        channel.postMessage({ type: 'TAB_CLOSED', path: '/branch-teller-interval' });
      };
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
                {!isStandalone && (
                  <button 
                  onClick={() => {
                    window.open('/branch-teller-interval', '_blank');
                    if (onClose) onClose();
                  }}
                  style={{
                      padding: '10px 20px',

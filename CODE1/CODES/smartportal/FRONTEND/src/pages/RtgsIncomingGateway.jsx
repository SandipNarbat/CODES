...
export default function RtgsIncomingGateway({ onClose }) {
  ...
  useEffect(() => {
    if (isStandalone) {
      ...
      const handleBeforeUnload = () => {
        channel.postMessage({ type: 'TAB_CLOSED', path: '/rtgs-incoming-gateway' });
      };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 style={{ marginTop: 0 }}>RTGS Incoming Gateway</h2>
            <div style={{ display: 'flex', gap: '10px' }}>
                {!isStandalone && (
                  <button 
                  onClick={() => {
                    window.open('/rtgs-incoming-gateway', '_blank');
                    if (onClose) onClose();
                  }}
                  style={{
                      padding: '10px 20px',
        ...
        <div style={{ marginTop: '1rem', marginBottom: '2rem', lineHeight: '1.5' }}>
            <p>
              This is a placeholder for the <strong>RTGS Incoming Gateway</strong> explanation.<br/>

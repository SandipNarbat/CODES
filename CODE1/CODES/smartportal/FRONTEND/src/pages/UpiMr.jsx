...
export default function UpiMr({ onClose }) {
  ...
  useEffect(() => {
    if (isStandalone) {
      ...
      const handleBeforeUnload = () => {
        channel.postMessage({ type: 'TAB_CLOSED', path: '/upi-mr' });
      };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 style={{ marginTop: 0 }}>UPI (MR)</h2>
            <div style={{ display: 'flex', gap: '10px' }}>
                {!isStandalone && (
                  <button 
                  onClick={() => {
                    window.open('/upi-mr', '_blank');
                    if (onClose) onClose();
                  }}
                  style={{
                      padding: '10px 20px',
        ...
        <div style={{ marginTop: '1rem', marginBottom: '2rem', lineHeight: '1.5' }}>
            <p>
              This is a placeholder for the <strong>UPI (MR)</strong> explanation.<br/>
